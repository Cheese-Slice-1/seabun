...
/// a position in code as: CodePos(LINE, COLUMN)
...
/// the types of token that exist
/// anything not defined here is marked as a TokenKind::Error
...
pub enum TokenKind {
    ...
    Do, // aka single-expression code block
    
    #[regex("get", priority=60)]
    Get,
    
    /* LITERAL-BASED TOKENS */
    ...
    #[regex(
        r#"("")|(")([^"\\\x00-\x1F]|\\(["\\bnfrt]|u[a-fA-F0-9]{4}|x[a-fA-F0-9]{2}))+?(")"#,
        priority = 40
    )]
    ...
    // these can refer to both type names and identifiers
    // you can guess correctly based on the context, of course:
        // Word Word => 100% identifier then type
        // Word = ... => 100% identifier
        // fun: Word Word -> Word  => 100% identifier, type, and type
    // this is because identifiers can't go after another identifier
    #[regex(r#"[\w][\w\d]*"#, priority = 20)]
    Word, // foo, bar_, _baz, bar2, seabun
    
    /// a parsing error OR an unknown token
    Error,
}
...
pub enum Expr {
    /* SIMPLE EXPRESSIONS */
    // these consist of a single non-self-referential value (aka plain values and that's it)
    ...
    /// enum
    Enum(/* how do i represent this..? */),
    
    /// function literal. e.g: fun: arg num do, fun -> chr do, fun: arg num -> chr do
    Fun {
    ...
    Bind {
        // e.g. let s = "". -> Bind {id: "s".into(), kind: (BindKind::Value, VarKind::Str), val: Box::new(Expr::Str("a".into()))}
        id: EcoString,
    ...
    ReBind {
        // e.g. a = 2. -> ReBind {id: "a".into(), val: Box::new(Expr::Num(2))}
        id: EcoString,
        val: Box<Expr>,
    },
}
...
pub enum VarKind {
    ...
    /*
        an example would be:
            ...
            let something_a = rec rec_a. ; rec: "", 0
            let something_b = rec rec_b. ; rec: "", 0
            show (something_a == something_b). ; "true"
    */
...
// is the def's variable kind unique?
#[derive(Clone, Debug, PartialEq)]
#[repr(u8)]
pub enum UniqueKind {
    Yes,
